...
const english = {
  ...
  presetFontWorkflowStep1: 'Choose one English/number font and one Chinese/system font.',
  presetFontWorkflowStep2: 'Edit menu → Apply Default Font applies the pair to the selected text only.',
  presetFontWorkflowStep3: 'Apply Fonts to Entire File applies the pair to every text layer.',
  ...
  fontAppliedSelected: 'Default font applied to the selected text.',
  fontAppliedAll: 'Default font applied to every text layer.',
...
const traditionalChinese = {
  ...
  presetFontWorkflowStep1: '先分別選擇一個英文/數字字型與一個中文/系統字型。',
  presetFontWorkflowStep2: '「編輯 → 套用預設字型」只套用到目前選取的文字區塊。',
  presetFontWorkflowStep3: '「套用字型至全檔」會把同一組字型套用到所有文字圖層。',
  presetFontWorkflowStep4: '「圖片載入/OCR 後強制套用」預設關閉；只有希望 OCR 產生後立刻改成預設字型時才啟用。',
  fontAppliedSelected: '已將預設字型套用到選取文字。',
  fontAppliedAll: '已將預設字型套用到所有文字圖層。',
...
const simplifiedChinese = { ...traditionalChinese, uiLang: 'UI 界面语言', file: '文件', edit: '编辑', view: '查看', settings: '设置', manualGuide: '操作说明', loadImage: '加载图片...', closeImage: '关闭图片', saveImage: '保存图片', exportCsv: '导出至 CSV', exportPdf: '导出为可搜索 PDF', exportImage: '导出图片', insertText: '插入文字', undo: '撤销', redo: '重做', showLeft: '显示左侧面板', showRight: '显示右侧面板', docStatus: '文件状态', noImage: '尚未加载图片', loaded: '✓ 图片已加载并处理完成', layers: '交互图层', noLayers: '尚未检测到图层', formatting: '文字格式设置', editContent: '编辑内容', placeholder: '请先选择一个文字区域', placeholderActive: '在此修改文字内容...', paragraph: '段落格式', bold: '粗体', italic: '斜体', color: '颜色预设', aiOps: 'AI 本地协同操作', fixText: '智能校对', extract: '提取实体', ops: '常用操作', regionalOcr: '区域 OCR', drawingMode: '框选绘图模式', eraseRegion: '框选删除', eraseDrawingMode: '框选删除模式', eraseRegionRunning: '正在删除框选内容…', removeText: '移除文字', presetFonts: '预设字体设置', engFont: '英文/数字字体', zhFont: '中文/系统字体', applyFontAllPreset: '应用字体至整个文件', loadDeviceFonts: '加载本机字体', fontLoading: '正在加载本机字体…', fontLoaded: '本机字体已加载。', fontUnsupported: '此浏览器不支持列出本机字体；如需完整本机字体清单，请使用桌面版 Chrome/Edge 或打包后的 App。', fontPermissionNeeded: '需要本机字体权限；若浏览器提示，请允许访问后再试一次。', fontPermissionDenied: '本机字体访问被拒绝或不可用；仍保留常用替代字体。', clickCanvasToInsertText: '请点击画布以放置新文字。', presetFontWorkflowTitle: '套用预设字体如何运作', presetFontWorkflowStep1: '先分别选择一个英文/数字字体与一个中文/系统字体。', presetFontWorkflowStep2: '“编辑 → 套用预设字体”只套用到当前选中的文字区块。', presetFontWorkflowStep3: '“应用字体至整个文件”会把同一组字体套用到所有文字图层。', presetFontWorkflowStep4: '“图片载入/OCR 后强制套用”默认关闭；只有希望 OCR 产生后立刻改成预设字体时才启用。', fontAppliedSelected: '已将预设字体套用到选中文字。', fontAppliedAll: '已将预设字体套用到所有文字图层。', fontApplyNoSelection: '尚未选择或没有可套用的文字图层。', localFontsHint: '浏览器获得本地字体权限后会纳入设备上的所有字体；不支持时仍保留常用替代字体。', ocrEngine: 'OCR 识别引擎', localEngine: '本地 (Tesseract)', cloudEngine: '云端 (Gemini AI)', nativeServerEngine: '本地服务器（原生 OCR）', rerunOcr: '🔄 重新识别（应用当前引擎）', recognizing: '识别中…', geminiKey: 'Gemini API 密钥', cloudModel: '云端模型', customBaseUrl: '自定义 API 节点（选填）', localServerUrl: '本地服务器地址 (URL)', connected: '已连接', checking: '正在检测...', serverNotFound: '未检测到服务器', testConnection: '测试连接', localServerPurpose: '本地服务器的用途', initializingAi: '正在初始化 AI 引擎（首次加载需下载约 950MB）...', initializingAiShort: '正在初始化 AI 引擎...', textCorrected: '文字校对完成！', entitiesExtracted: '实体提取完成！', aiFailed: 'AI 协同处理失败。', noLayersExport: '没有可导出的图层。', loadImageFirst: '请先加载一张图片。' };
